...
update_gitignore() {
    ...
    if ! grep -q "nss/.*\.db" "$temp_gitignore" 2>/dev/null; then
        echo "" >> "$temp_gitignore"
        echo "# NSS databases (generated at runtime)" >> "$temp_gitignore"
        echo "nss/*/cert9.db" >> "$temp_gitignore"
        echo "nss/*/key4.db" >> "$temp_gitignore"
        echo "nss/*/pkcs11.txt" >> "$temp_gitignore"
    fi
