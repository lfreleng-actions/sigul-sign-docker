...
test_container_hostnames() {
    ...
    local bridge_hostname=$(docker exec sigul-bridge hostname)
    local server_hostname=$(docker exec sigul-server hostname)
...
test_container_static_ips() {
    ...
    local bridge_ip=$(docker inspect sigul-bridge | grep -o '"IPAddress": "[^"]*' | grep -o '[^"]*$' | head -1)
    local server_ip=$(docker inspect sigul-server | grep -o '"IPAddress": "[^"]*' | grep -o '[^"]*$' | head -1)
...
test_network_subnet() {
    ...
    local subnet=$(docker network inspect sigul-network | grep -o '"Subnet": "[^"]*' | grep -o '[^"]*$' | head -1)
