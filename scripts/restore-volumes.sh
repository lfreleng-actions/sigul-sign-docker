...
check_services_stopped() {
    ...
    local running_containers=$(docker ps --filter "name=sigul" --format "{{.Names}}" 2>/dev/null || true)
...
get_volume_name_from_file() {
    ...
    local volume_name=$(echo "$filename" | sed -E 's/-[0-9]{8}-[0-9]{6}\.tar\.gz$//')
...
restore_all_volumes() {
    ...
    local backup_files=($(get_backup_files "$timestamp"))
    ...
    for file in "${backup_files[@]}"; do
        local volume=$(get_volume_name_from_file "$file")
