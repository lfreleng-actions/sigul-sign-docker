        ...
        for cfg in /etc/sigul/*.conf; do
            if [ -f "$cfg" ]; then
                ...
                cp "$cfg" "$output/$basename" 2>/dev/null || {
                    sudo cat "$cfg" > "$output/$basename" 2>/dev/null || echo "Cannot read $cfg" > "$output/$basename"
                }
            fi
        done
    fi
...
extract_certificates() {
    ...
    sudo certutil -L -d /etc/pki/sigul 2>/dev/null | tail -n +4 | awk '{print $1}' | while read nick; do
        if [ -n "$nick" ] && [ "$nick" != "" ]; then
            local safe_filename=$(echo "$nick" | tr '/' '_' | tr ' ' '_')
...
extract_systemd_services() {
    ...
    sudo systemctl list-units 'sigul_server@*' --no-legend 2>/dev/null | awk '{print $1}' | while read instance; do
        if [ -n "$instance" ]; then
            local safe_name=$(echo "$instance" | tr '@' '_' | tr '.' '_')
...
extract_nss_details() {
    ...
    # NSS configuration files
    {
        ...
        if [ -f /etc/pki/sigul/nss-password.txt ]; then
            echo "File exists, length: $(sudo wc -c < /etc/pki/sigul/nss-password.txt) bytes"
        else
...
extract_logging() {
    ...
    sudo find /var/log/sigul* -type f -name "*log" 2>/dev/null | while read logfile; do
        local safe_name=$(echo "$logfile" | tr '/' '_')
...
extract_process_info() {
    ...
    # Running processes
    {
        ...
        ps aux | grep -E '[s]igul|[p]ython.*sigul' || echo "No Sigul processes"
