...
get_container_hostname() {
    ...
    local hostname=$(docker exec "sigul-${COMPONENT}" hostname)
...
get_cert_nickname() {
    ...
    local cert_nickname=$(docker exec "sigul-${COMPONENT}" \
...
get_cert_subject() {
    ...
    local subject=$(docker exec "sigul-${COMPONENT}" \
    ...
    if [ -n "$subject" ]; then
        ...
        # Extract CN value
        local cn=$(echo "$subject" | grep -oP 'CN=[^,]+' | cut -d= -f2 || echo "")
...
get_cert_sans() {
    ...
    local cert_details=$(docker exec "sigul-${COMPONENT}" \
        certutil -L -n "${cert_nickname}" -d "sql:${nss_db_path}" 2>/dev/null)

    # Extract SAN section
    local sans=$(echo "$cert_details" | grep -A 20 "DNS name:" | grep "DNS name:" || echo "")

    if [ -n "$sans" ]; then
        echo "  Subject Alternative Names:"
        echo "$sans" | while read line; do
            echo "    ${line}"
        done
        echo "$sans"
        return 0
...
verify_cert_validity() {
    ...
    local not_after=$(echo "$cert_details" | grep "Not After" || echo "")
...
main() {
    ...
    local hostname=$(get_container_hostname)
    echo ""

    local cert_nickname=$(get_cert_nickname)
    if [ $? -ne 0 ]; then
        fatal "Cannot proceed without certificate nickname"
    fi
    echo ""

    local nss_db_path=$(get_nss_db_path)
    ...
    # Get certificate details
    local cn=$(get_cert_subject "$cert_nickname" "$nss_db_path")
    echo ""

    local sans=$(get_cert_sans "$cert_nickname" "$nss_db_path" || echo "")
    echo ""
