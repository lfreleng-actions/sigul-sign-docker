...
verify_established_connections() {
    ...
    local bridge_conns=$(docker exec sigul-bridge netstat -tnp 2>/dev/null | grep ":44333" | grep "ESTABLISHED" || echo "")

    if [ -n "$bridge_conns" ]; then
        echo "$bridge_conns" | while read line; do
            echo "    ${line}"
        done
    ...
    local server_conns=$(docker exec sigul-server netstat -tnp 2>/dev/null | grep ":44333" | grep "ESTABLISHED" || echo "")

    if [ -n "$server_conns" ]; then
        echo "$server_conns" | while read line; do
            echo "    ${line}"
        done
...
verify_docker_network() {
    ...
    # Get network details
    local subnet=$(docker network inspect sigul-network | jq -r '.[0].IPAM.Config[0].Subnet' 2>/dev/null)
    local gateway=$(docker network inspect sigul-network | jq -r '.[0].IPAM.Config[0].Gateway' 2>/dev/null)
...
show_container_ips() {
    ...
    local bridge_ip=$(docker inspect sigul-bridge | jq -r '.[0].NetworkSettings.Networks."sigul-network".IPAddress' 2>/dev/null)
    local server_ip=$(docker inspect sigul-server | jq -r '.[0].NetworkSettings.Networks."sigul-network".IPAddress' 2>/dev/null)
...
verify_port_forwarding() {
    ...
    local exposed_ports=$(docker port sigul-bridge 2>/dev/null || echo "")
...
verify_routing() {
    ...
    docker exec sigul-bridge ip route 2>/dev/null | head -5 | while read line; do
        echo "    ${line}"
    done

    echo ""
    echo "  Server routing table:"
    docker exec sigul-server ip route 2>/dev/null | head -5 | while read line; do
        echo "    ${line}"
    done
