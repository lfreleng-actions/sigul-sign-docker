...
verify_self_resolution() {
    ...
    if docker exec "sigul-${COMPONENT}" getent hosts "${expected_hostname}" &>/dev/null; then
        local resolved=$(docker exec "sigul-${COMPONENT}" getent hosts "${expected_hostname}")
        echo "  ${resolved}"
...
verify_bridge_resolution() {
    ...
    if docker exec sigul-server getent hosts sigul-bridge.example.org &>/dev/null; then
        local resolved=$(docker exec sigul-server getent hosts sigul-bridge.example.org)
        echo "  ${resolved}"
...
show_hosts_file() {
    ...
    echo ""
    docker exec "sigul-${COMPONENT}" cat /etc/hosts | grep -v "^#" | grep -v "^$" | while read line; do
        echo "  ${line}"
    done
    echo ""
}
...
verify_short_name() {
    ...
    if docker exec "sigul-${COMPONENT}" getent hosts "${short_name}" &>/dev/null; then
        local resolved=$(docker exec "sigul-${COMPONENT}" getent hosts "${short_name}")
        echo "  ${resolved}"
...
verify_network_aliases() {
    ...
    local container_id=$(docker ps --filter "name=sigul-${COMPONENT}" --format "{{.ID}}")
    ...
    local aliases=$(docker inspect "$container_id" \
...
verify_static_ip() {
    ...
    local container_ip=$(docker inspect "sigul-${COMPONENT}" \
