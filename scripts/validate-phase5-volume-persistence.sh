...
test_volumes_exist() {
    ...
    local volume_count=$(docker volume ls --filter "name=sigul_" --format "{{.Name}}" | wc -l)
...
test_volume_labels_applied() {
    ...
    local volumes_with_labels=$(docker volume ls --filter "name=sigul_" --format "{{.Name}}" | head -1)
